...
class MRIbex(MRJob):
    ...
    def reducer(self, key, data):
        ...
        if len(maximo_incremento_mes) != 0 and len(maximo_incremento_semana) != 0:
            ...
            ordenado_por_valor_mes = dict(sorted(maximo_incremento_mes.items(), key=lambda x:x[1], reverse=True))
            ordenado_por_valor_semana = dict(sorted(maximo_incremento_semana.items(), key=lambda x:x[1], reverse=True))
    ...
    def funcionMinMax(self, ultimo_valor, llave_accion, maximo_incremento):
        if len(maximo_incremento.items()) == 0:
            ...
            maximo_incremento[llave_accion] = ultimo_valor
            maximo_incremento = dict(sorted(maximo_incremento.items(), key=lambda x:x[1], reverse=True))
